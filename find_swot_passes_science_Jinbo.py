...
if os.path.exists(filepath+"/sph_science_nadir.zip") is False or os.path.exists(filepath+"/sph_science_swath.zip") is False:
...
#parse the command line arguments w
...
# sw_corner = [-77,35]#args.sw_corner
# ne_corner = [-60,47] #args.ne_corner
# cycles = [1,31]
...
def find_time(ID_PASS_value, extended_bbox):
    ...
    """
    nadir_data = gpd.read_file(filepath+"/sph_science_nadir.zip")
...
# Load the shapefile
gdf_karin = gpd.read_file(filepath+"/sph_science_swath.zip")
...
for cycle in np.arange(1,32):
    # overlapping_segments["TIME_1"] = overlapping_segments["TIME"] + cycle_period
    overlapping_segments[f"TIME_{cycle}"] = overlapping_segments["TIME"] + (cycle * cycle_period)
    # overlapping_segments[f"TIME_{cycle}"] = overlapping_segments["TIME"] + (cycle * cycle_period)

# safe passIDs and pass times as text file
overlapping_segments.to_csv(f'passes_{sw_corner[0]}W_{sw_corner[1]}N_{ne_corner[0]}W_{ne_corner[1]}N_cycles{cycles[0]}_{cycles[1]}.txt', sep='\t', index=False)
# only save passIDs to textfile
overlapping_segments['ID_PASS'].to_csv(f'passIDs_{sw_corner[0]}W_{sw_corner[1]}N_{ne_corner[0]}W_{ne_corner[1]}N.txt', index=False)
...
print(overlapping_segments[["ID_PASS", f"TIME_{cycle[0]}",f"TIME_{cycle[1]}"]])
...
fig, axes = plt.subplots(nrows=n_rows, ncols=6, figsize=(15,13), subplot_kw={'projection': ccrs.PlateCarree()})
axes = axes.ravel()
